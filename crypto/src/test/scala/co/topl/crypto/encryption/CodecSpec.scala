...
class CodecSpec extends AnyPropSpec with ScalaCheckDrivenPropertyChecks with Matchers {
  ...
  property("Cipher > AES > Encode and Decode") {
    // Decode test
    val testParams = Helpers.ExpectedCipher.json.as[cipher.Cipher[Id]]
    testParams.isRight shouldBe true
    testParams shouldBe Right(Helpers.ExpectedCipher.value)
    // Encode test
  ...
  property("KDF > SCrypt > Encode and Decode") {
    // Decode test
    val testParams = Helpers.ExpectedKdf.json.as[kdf.Kdf[Id]]
    testParams.isRight shouldBe true
    testParams shouldBe Right(Helpers.ExpectedKdf.value)
    // Encode test
  ...
  property("VaultStore > Encode and Decode") {
    // Decode test
    val testParams = Helpers.ExpectedVaultStore.json.as[VaultStore[Id]]
    testParams.isRight shouldBe true
    testParams shouldBe Right(Helpers.ExpectedVaultStore.value)
    // Encode test
