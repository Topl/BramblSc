...
import co.topl.brambl.syntax.{ioTransactionAsTransactionSyntaxOps, LvlType}
...
class BifrostMonitorTest extends munit.CatsEffectSuite {
      ...
      BifrostMonitor("localhost", 9184, secureConnection = false, bifrostQuery1).use { blockStream =>
        (for {
          ...
          queriedHeights <- blockUpdates
            .map(b => bifrostQuery1.blockByHeight(b._2).map(r => (r.get._4.map(_.computeId), r.get._2.height)))
            .sequence
  ...
  test("Monitor live and retroactive blocks") {
    ...
    assertIO(
      BifrostMonitor("localhost", 9184, secureConnection = false, bifrostQuery1, Some(startingBlockId)).use {
        blockStream =>
