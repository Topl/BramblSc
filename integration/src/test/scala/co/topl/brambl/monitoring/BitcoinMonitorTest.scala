...
class BitcoinMonitorTest extends munit.CatsEffectSuite {
    ...
    assertIO(
      for {
        ...
        node1MintBlocks <- IO.fromFuture(
          IO(bitcoindInstance.getNewAddress(TestWallet).flatMap(bitcoindInstance.generateToAddress(1, _)))
        )
        node2MintBlocks <- IO.fromFuture(
          IO(node2Instance.getNewAddress(TestWallet).flatMap(node2Instance.generateToAddress(2, _)))
        )
        _ <- IO
          .fromFuture(IO(bitcoindInstance.addNode(bitcoind.bitcoindInstance2Uri, AddNodeArgument.Add)))
          .attempt
          .andWait(5.seconds)
        additionalMintBlocks <- IO.fromFuture(
          IO(node2Instance.getNewAddress(TestWallet).flatMap(node2Instance.generateToAddress(1, _)))
        )
    ...
    assertIO(
      for {
        ...
        mintBlocks <- IO.fromFuture(
          IO(bitcoindInstance.getNewAddress(TestWallet).flatMap(bitcoindInstance.generateToAddress(NumBlocks, _)))
        )
        ...
        val startingHeight = blocks.head.height
        blocks.map(_.block.blockHeader.hashBE).toVector == mintBlocks && blocks.map(_.height) == mintBlocks.zipWithIndex
          .map(_._2 + startingHeight)
      },
      true
    )
  }
  ...
  test("Monitor new blocks and report existing blocks") {
    ...
    val existingBlocks = Await.result(
      bitcoindInstance.getNewAddress(TestWallet).flatMap(bitcoindInstance.generateToAddress(NumBlocks, _)),
      5.seconds
    )
