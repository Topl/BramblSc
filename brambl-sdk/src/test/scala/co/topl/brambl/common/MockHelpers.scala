...
trait MockHelpers {
  ...
  val inLock: Lock.Predicate = Lock.Predicate(List(
  ...
  val txDatum: Datum.IoTransaction = Datum.IoTransaction(Event.IoTransaction(
    Schedule(3, 50, 100).some,
    List(), List(),
    SmallData(ByteString.copyFrom("metadata".getBytes)).some).some
  )
  ...
  val inputFull: SpentTransactionOutput = SpentTransactionOutput(knownId.some, attFull.some, value.some, inDatum.some, List())

  val attEmpty: Attestation = Attestation().withPredicate(Attestation.Predicate(inLock.some, inResponsesEmpty))
  val inputEmpty: SpentTransactionOutput = SpentTransactionOutput(knownId.some, attEmpty.some, value.some, inDatum.some, List())
