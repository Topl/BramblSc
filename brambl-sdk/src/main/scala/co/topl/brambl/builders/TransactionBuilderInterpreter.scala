...
object TransactionBuilderInterpreter {
  def make[F[_]: Monad](dataApi: DataApi): TransactionBuilder[F] = new TransactionBuilder[F] {
    override def constructUnprovenInput(data: InputBuildRequest): F[Either[BuilderError.InputBuilderError, SpentTransactionOutput]] = {
      val id = data.id
      ...
      (id, attestation, value) match {
        case (Some(knownId), Some(Right(att)), Some(boxVal)) =>
          SpentTransactionOutput(knownId.some, att.some, boxVal.some, datum.some, opts).asRight[BuilderError.InputBuilderError].pure[F]
        case (_, Some(Left(err)), _) => err.asLeft[SpentTransactionOutput].pure[F]
        case _ => BuilderError.InputBuilderError(
            s"Could not construct input. Id=${id}, Attestation=${attestation}, Value=${value}"
          ).asLeft[SpentTransactionOutput].pure[F]
      }
    }

    override def constructOutput(data: OutputBuildRequest): F[Either[BuilderError.OutputBuilderError, UnspentTransactionOutput]] = {
      ...
      (data.lock, data.value) match {
        case (Some(lock), Some(value)) =>
          ...
          UnspentTransactionOutput(address.some, value.some, datum.some).asRight[BuilderError.OutputBuilderError].pure[F]
        case _ =>
          BuilderError.OutputBuilderError(s"Could not construct output. Lock=${data.lock}, Value=${data.value}").asLeft[UnspentTransactionOutput].pure[F]
      }
    }

    override def constructUnprovenTransaction(inputRequests: List[InputBuildRequest], outputRequests: List[OutputBuildRequest],
                                              schedule: Option[Schedule], output32Refs: List[KnownIdentifier.TransactionOutput32],
                                              output64Refs: List[KnownIdentifier.TransactionOutput64], metadata: Option[SmallData]):
    F[Either[List[BuilderError], IoTransaction]] = {
      ...
      val inputs = inputRequests
        ...
        // only works if F is an Applicative (Monad and Validated is)
      ...
      for {
        ...
        case (Validated.Invalid(errsI), Validated.Invalid(errsO)) => (errsI.toList ++ errsO.toList).asLeft[IoTransaction]
