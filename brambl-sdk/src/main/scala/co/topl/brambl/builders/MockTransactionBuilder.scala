...
object MockTransactionBuilder extends TransactionBuilder {
  ...
  override def constructOutput(
  ...
  ): Either[BuilderError.OutputBuilderError, UnspentTransactionOutput] = {
    ...
    (data.lock, data.value) match {
      ...
      case _ =>
        Left(BuilderError.OutputBuilderError(s"Could not construct output. Lock=${data.lock}, Value=${data.value}"))
    }
  }

  override def constructUnprovenInput(
    data: InputBuildRequest
  ): Either[BuilderError.InputBuilderError, SpentTransactionOutput] = {
    val id = data.id
    ...
    (id, attestation, value) match {
      ...
      case _ =>
        Left(
          BuilderError.InputBuilderError(
