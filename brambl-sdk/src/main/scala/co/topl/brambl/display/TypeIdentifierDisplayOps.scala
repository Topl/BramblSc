...
trait TypeIdentifierDisplayOps {
  ...
  implicit val typeIdentifierDisplay: DisplayOps[ValueTypeIdentifier] = {
    ...
    case AssetType(groupIdOrAlloy, seriesIdOrAlloy) => s"Asset(${Encoding.encodeToHex(groupIdOrAlloy.toByteArray)}, ${Encoding.encodeToHex(seriesIdOrAlloy.toByteArray)})"
