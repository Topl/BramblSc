...
The SDK provides an easy way to share templates with other users using JSON.
...
Type parameters:
- `F`: The context, bound to a context parameter of type `Monad[F]`, in which the template will be encoded. For example, if you
are using Cats Effect `IO`:
  ```scala
  import cats.effect.IO

  val jsonTemplate = encodeLockTemplate[IO](template)
  ```
...
Type parameters:
- `F`: The context, bound to a context parameter of type `Monad[F]`, in which the template will be decoded. For example, if you
are using Cats Effect `IO`:
  ```scala
  import cats.effect.IO

  val lockTemplate = decodeLockTemplate[IO](template)
  ```
...
function of a Lock Template instance.
...
SignatureTemplates contained in the Lock Template.
...
the <ScaladocLink path="lockAddress(Lock):F[LockAddress]"><code>lockAddress</code></ScaladocLink>
...
The SDK provides an easy way to share Lock Addresses with other users using a Base58 encoding scheme.
